...
interface Props {
	navigation: NativeStackNavigationProp<ParamListBase, '매장추가'>;
}
...
function CreateStore({ navigation }: Props): ReactElement {
	...
	const finish = async () => {
		...
		if (data.validation === true) {
			Alert.alert('완료', '매장이 추가되었습니다.', [{ text: '확인', onPress: () => navigation.navigate('관리매장') }]);
			setManager(true);
		} else {
			Alert.alert('실패', '유감입니다.', [{ text: '확인', onPress: () => navigation.navigate('관리매장') }]);
		}
	};
